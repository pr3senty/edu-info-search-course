...
def build_or_resume_index(index_dir: Path, docs_dir: Path, verbose: bool = True):
    ...
    index = InvertedIndex(lsm, analyzer, kgram_index=kgram, documents_count=documents_count)
    index.rebuild_kgram()
...
HELP = """Commands:
  ...
  prefix <text>        Search documents by term prefix
  wildcard <pattern>   Search documents by wildcard pattern using k-gram
...
def run_repl(lsm, index, docs_map, index_dir: Path):
    ...
    while True:
        ...
        if s.startswith("prefix "):
            prefix = s[len("prefix "):].strip()
            try:
                res = index.prefix_query(prefix)
                ids = list(res)
                print(f"Matched {len(ids)} docs")
                print(ids)
            except Exception as e:
        ...
        if s.startswith("wildcard "):
            pattern = s[len("wildcard "):].strip()
            try:
                terms = index.wildcard_terms(pattern, limit=32)
                print("Matched terms:", [term for term in terms])

                res = index.wildcard_query(pattern)
                ids = list(res)
                print(f"Matched {len(ids)} docs")
                print(ids)
            except Exception as e:
