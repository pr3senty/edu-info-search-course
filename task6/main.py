...
def build_or_resume_index(index_dir: Path, docs_dir: Path, verbose: bool = True):
    ...
    lsm.flush()
    retriever = RankedRetriever(index)
    retriever.build()
    ...
    return lsm, index, retriever, docs_map


HELP = """Commands:
  query <expr>        Boolean query. Example: query (привет AND мир) OR python
  phrase <text>       Exact phrase query. Example: phrase структуры данных
  ranked <text>       Ranked free text query. Example: книги про программирование
...
def run_repl(lsm, index: PositionalIndex, retriever: RankedRetriever, docs_map, index_dir: Path):
    print(HELP)
    ...
    while True:
        ...
        if s.startswith("ranked "):
            q = s[len("ranked "):].strip()
            results = retriever.rank_query_inexact(q, k=10)
            print(results)
            continue
...
def main():
    ...
    lsm = None
    try:
        lsm, index, retriever, docs_map = build_or_resume_index(index_dir, docs_dir, verbose=True)
        run_repl(lsm, index, retriever, docs_map, index_dir)
    finally:
        if lsm is not None:
            lsm.flush()
            lsm.close()
